...
def _populate_map(conf_map: dict[str, t.Any], full_config: dict[str, t.Any]) -> None:
...
def load_upadup_config() -> dict[str, t.Any]:
    ...
    if upadup_config.get("extends_default", True):
        _populate_map(upadup_config_map, DEFAULT_CONFIG)
    _populate_map(upadup_config_map, upadup_config)

    return {"repos": upadup_config_map}
